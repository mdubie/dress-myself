(function() {
  ...
  //potential inputs: multiple arguments
...
/*
    ...
    if (Array.isArray(collection))  {
      for(var i = 0; i < collection.length; i++)  {
        mapped[i] = iterator(collection[i]);
      }
    } else for(var j in collection)  {
      mapped[j] = iterator(collection[j]);
    }
    */
  ...
  _.memoize = function(func) {
    ...
    return function(){
      ...
      if (!(args in history)){
        history[args] = func.apply(this,arguments);
      }
  ...
  // build an object that categorizes all elements
  // check that each array has each element
  ...
  _.intersection = function() {
    ...
    _.each(args,function(array){
      _.each(array,function(element){
        categorize[element] = 1;
      })
    })
    var temp = true;
    for (var element in categorize){
      for (var i = 0; i < args.length; i++){
        temp = Boolean(temp) && Boolean((_.indexOf(args[i],element) !== -1));
      }
      if (temp) {
        output.push(element);
      }
